...
must_haves:
  ...
  key_links:
    ...
    - from: "src/components/odyssey-world/hooks/useWorldMap.ts"
      to: "src/components/odyssey-world/hooks/useRegionTransition.ts"
      via: "fastTravel calls initiateTransition for region switching"
      pattern: "initiateTransition"
---
...
<task type="auto">
  ...
  <action>
    ...
    **Update useDiscoveryState.ts:**
    - After `achieveDiscovery` is called, also check discovery connections reactively: extract a plain function `checkDiscoveryConnections(achievedDiscoveries, allRegionDefinitions)` from `useDiscoveryConnections.ts` (NOT a hook call) and invoke it from the store action, OR use `store.subscribe()` in `useDiscoveryConnections` to reactively detect `achievedDiscoveries` changes
