...
1. ✅ **实验噪声模型** - 改用高斯分布 + 散粒噪声模型 (`dataAnalysis.ts`)
2. ✅ **光学工作台偏振片** - 现在使用非理想器件参数 (`opticalBenchStore.ts`)
3. ✅ **游戏简化模型警告** - 添加了UI提示 (`HelpPanel.tsx`, `en.json`, `zh.json`)
4. ✅ **DataPoint接口** - 添加了不确定度字段 (`labStore.ts`)
5. ✅ **Malus's Law统一** - 游戏引擎现在使用共享库 (`game2d/physics.ts`)
