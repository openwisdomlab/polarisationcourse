...
interface PolarizerVisualizerProps {
  ...
  isActive: boolean
  showLines?: boolean
  themeColors?: ReturnType<typeof useDemoTheme>
}
  ...
  return (
    <g transform={`translate(${x}, 150)`}>
      <motion.ellipse
        ...
        rx={radius}
        ry={radius * 1.2}
        fill={isActive ? `${color}15` : (themeColors?.polarizerInactiveFill ?? '#1e293b')}
        stroke={isActive ? color : (themeColors?.polarizerInactiveStroke ?? '#475569')}
          ...
          {Array.from({ length: lineCount }, (_, i) => {
            ...
            return (
              <line
                ...
                x2={xLength}
                y2={yOffset}
                stroke={isActive ? color : (themeColors?.textMuted ?? '#64748b')}
                strokeWidth={1.5}
      ...
      <text x="0" y={radius + 25} textAnchor="middle" fill={themeColors?.textSecondary ?? '#9ca3af'} fontSize="11">
        {label}
      </text>
          ...
          >
            ...
            <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <InfoCard title={isZh ? '圆偏振' : 'Circular Polarization'} color={polarizationType === 'circular' ? 'green' : 'cyan'}>
                <p className={`text-xs ${dt.bodyClass}`}>{isZh ? '电场矢量端点描绘圆形轨迹，等振幅、相位差90°。' : 'E-field tip traces a circle, equal amplitudes with 90° phase difference.'}</p>
              </InfoCard>
